...
const LegScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (userId) {
          data = await loadUserRoutine(userId, 'LEG');
        }
        const source = data || getDefaultExercises('LEG');
        ...
        const defaults = getDefaultExercises('LEG');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const success = await saveUserRoutine(userId, 'LEG', dataToSave);
      if (success) {
        savedSnapshotRef.current = getSnapshot(exercises);
        Alert.alert('Saved! ✅', 'Your Leg routine has been updated.');
      } else {
